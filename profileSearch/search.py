...
df = pd.DataFrame(columns=["Item", "Profile"])
...
model = "/Users/andrew/Documents/dev/text-generation-webui/models/llama-2-7b-chat.Q4_K_M.gguf"
...
for name in names:
    ...
    input_q = f"Describe the flavor profile of this product using only single words seperated by commas: {name}. {example}"
    ...
    df.to_csv("llama_profiles_disp.csv")
